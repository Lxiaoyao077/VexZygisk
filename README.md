...
`module.prop` points the manager at `update.json` (KernelSU) or `update-apatch.json` (APatch) on this repository's `main` branch. Both files are **generated by CI, never edited by hand**: `scripts/make_update_json.py` picks the newest release archive of the matching flavour, embeds its version, `versionCode`, download URL and the changelog, and warns when the version tag does not exist yet or does not point at the current HEAD.

They are not committed automatically. When cutting a release:

1. Download the `update.json` and `update-apatch.json` artifacts from the green Trusted CI run that built the archives you are releasing.
2. Commit them at the repository root (so the `raw.githubusercontent.com` URLs resolve) and publish the release with the tag the files name — cutting the tag after the commit keeps the tag check green on the next run.
