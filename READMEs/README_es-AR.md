...
La elección de la build/zip es importante, ya que va a determinar qué tan oculto y estable sea VexZygisk. Igual, esto no es una tarea difícil:
...
Después de elegir la build correcta, tendrías que flashearla usando la app de KernelSU. Podés hacer esto yendo a la sección de `Módulos` (`Modules`) de tu gestor de root y seleccionando el zip que te bajaste.
...
Después de reiniciar, podés verificar si VexZygisk está funcionando bien revisando la descripción del módulo en la sección de `Módulos` de tu gestor de root. La descripción debería indicar que los demonios (daemons) necesarios están corriendo. Debería verse parecido a esto: `[Monitor: ✅, VexZygisk 64-bit: ✅] Standalone implementation of Zygisk.`
...
Actualmente hay dos maneras diferentes de aportar con traducciones para VexZygisk:
...
Es obligatorio seguir las [Guías de Contribución](https://github.com/PerformanC/contributing) de PerformanC para aportar a VexZygisk. Esto incluye respetar su Política de Seguridad, su Código de Conducta y el estándar de sintaxis.

## Licencia

VexZygisk está bajo la licencia [AGPL 3.0](./LICENSE). Podés leer más sobre esto en la [Open Source Initiative](https://opensource.org/licenses/AGPL-3.0).
