/* INFO: Zygisk Next module API, taken from NyaZygisk (HSSkyBoy) which itself
         follows the Zygisk Next and ZygiskNextNext module contract. Kept as is
         so modules compiled against it link and run unmodified. */
...
struct ZygiskNextCompanionModule {
    ...
    /* INFO: Written as (void) rather than () because the daemon is built as
             C99 with -Wstrict-prototypes. It is the same signature, so
             modules compiled against either spelling stay compatible. */
    void (*onCompanionLoaded)(void);
